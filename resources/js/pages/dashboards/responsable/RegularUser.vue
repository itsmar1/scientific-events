...
export default {
  ...
  computed: {
    users() {
      return this.$store.getters["responsable/users"];
    },

  },
  methods: {
    async getUsers() {
      await this.$store.dispatch('responsable/getUsers');
    },
    updateUser(payload){
        this.$store.dispatch('responsable/postUsers', {
            user: payload,
            id: payload.id
        });
    },
    deleteUser(id) {
        this.$store.dispatch('responsable/deleteUsers', id);
    },
    async addUser() {
        await this.$store.dispatch('responsable/postUsers', this.user);
    }
  },
  mounted() {
    this.getUsers();
