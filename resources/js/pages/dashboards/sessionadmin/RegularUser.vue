...
export default {
  ...
  computed: {
    users() {
      return this.$store.getters["sessionadmin/users"];
    },

  },
  methods: {
    async getUsers() {
      await this.$store.dispatch('sessionadmin/getUsers');
    },
    updateUser(payload){
        this.$store.dispatch('sessionadmin/postUsers', {
            user: payload,
            id: payload.id
        });
    },
    deleteUser(id) {
        this.$store.dispatch('sessionadmin/deleteUsers', id);
    },
    async addUser() {
        await this.$store.dispatch('sessionadmin/postUsers', this.user);
    }
  },
  mounted() {
    this.getUsers();
