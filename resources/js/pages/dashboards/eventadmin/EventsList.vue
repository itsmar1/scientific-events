...
export default {
  ...
  computed: {
      events(){
          const events = this.$store.getters['eventadmin/events'];
  ...
  methods: {
    async loadEvents() {
      await this.$store.dispatch('eventadmin/events');
      // test
    ...
    submitForm(){
            ...
            this.$store.dispatch('eventadmin/postEvent', { event: this.event, image: data});
    ...
    updateEvent(payload)
    {
        this.$store.dispatch('eventadmin/putEvent', payload);
    }
  },
  created() {
    this.loadEvents();
