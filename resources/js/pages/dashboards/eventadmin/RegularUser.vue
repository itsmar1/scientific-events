...
export default {
  ...
  computed: {
    users() {
      return this.$store.getters["eventadmin/users"];
    },

  },
  methods: {
    async getUsers() {
      await this.$store.dispatch('eventadmin/getUsers');
    },
    updateUser(payload){
        this.$store.dispatch('eventadmin/postUsers', {
            user: payload,
            id: payload.id
        });
    },
    deleteUser(id) {
        this.$store.dispatch('eventadmin/deleteUsers', id);
    },
    async addUser() {
        await this.$store.dispatch('eventadmin/postUsers', this.user);
    }
  },
  mounted() {
    this.getUsers();
